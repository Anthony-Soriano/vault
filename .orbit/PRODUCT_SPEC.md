...
**Planned (see `.orbit/ROADMAP.md`):** AI + Project Truth Engine (Phase 3, active — delivered incrementally as `v0.3.0` AI Foundation → `v0.3.1` Project Context & Repository Analysis → `v0.3.2` Project Truth Bootstrap → `v0.3.3` AI Proposal Review & Approval → `v0.3.4` Knowledge Proposal Engine → `v0.3.5` Project Truth Maintenance); derived Project DNA (Phase 4); semantic drift detection (Phase 5); stable public API (Phase 6). `v0.3.0` (internal AI plumbing) and `v0.3.1` (Project Context & Repository Analysis — deterministic, local-first evidence discovery, Project Truth readiness detection, transparent context packaging, and a read-only inspection surface, with no Project Truth generation or canonical mutation) are both complete and tagged; Phase 3 is between slices (see `.orbit/ARCHITECTURE.md`). Only the slice named active in `.orbit/CURRENT_PHASE.md` is approved implementation scope. Deferred robustness items in `.orbit/BACKLOG.md`.
