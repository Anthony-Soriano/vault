...
### `v0.3.2` (Project Truth Bootstrap) is bootstrap + structural gap-fill only, proposal-only, and ephemeral
Owner activated `v0.3.2` on 2026-08-07 (design/planning) and it is now **complete and verified** (`v0.3.0`/`v0.3.1` stay complete/locked; `v0.3.3`–`v0.3.5` stay planned/inactive). Bootstrap drafts **missing** Project Truth from repository evidence via the `v0.3.0` proposal pipeline + `v0.3.1` context analysis. Four locked owner decisions (2026-08-07) govern it: **(1) existing-stack boundary** — bootstrap + structural gap-fill only; existing docs stay authoritative by default; deep staleness/change/maintenance proposals belong to `v0.3.5` (no maintenance engine pulled in); **(2) ephemeral drafts** — no proposal store/DB, no canonical writes, no approve/edit/save/merge/replace execution (those are `v0.3.3`); **(3) evidence validation** — a citation supports a technical fact only if its `ref` resolves to evidence present in the supplied context inventory; unverifiable refs are downgraded to owner-input-needed, never presented as evidence-backed; **(4) existing-state disposition** — present/complete Project Truth defaults to `keep_existing` with no change/replacement recommendation. **Why:** incremental Phase 3 delivery keeps each boundary verifiable and preserves every trust invariant; keeping v0.3.2 to *drafting missing* Project Truth prevents scope creep into `v0.3.3` (review/persistence) and `v0.3.5` (maintenance). (Concrete scope/acceptance/verification live in `.orbit/CURRENT_PHASE.md`.)

### Planner is the sole authority over bootstrap scope (binding invariant)
`planProjectTruthBootstrap` alone decides which documents exist, which are targeted, their state, and the generation scope. The AI fills planner-selected slots only — it may **never** invent an additional Project Truth document nor silently skip a planner-selected target. Enforced structurally: `mapBootstrapDrafts` iterates the plan's targets (never the provider's proposals); unmatched proposals are discarded, unmatched targets still emit a `proposal: null` draft; `drafts.length === plan.targets.length`. **Why:** the AI is the first non-user writer of proposals; making the deterministic planner the scope authority guarantees a fallible model can never expand or silently shrink the canonical document set.

### Per-target generation; proposal identity from the target, never response order (owner decision D1, 2026-08-07)
The bootstrap service issues **one `AiService.propose` call per planner-selected create-target**, using that target's `purpose`/`instructions`; each call's proposals are attributed to that target by the target's own document identity. There is **no single merged call and no positional (nth-proposal↔nth-target) pairing**. **Why:** prioritizes correctness, exact attribution, and trust over minimizing model calls. Model-call batching is a permitted future optimization only if it preserves these semantics (never reintroduces positional pairing).

### The bootstrap IPC channel is non-mutating and uses a direct async handler
`vault:project-truth:bootstrap` is registered with a direct `ipcMain.handle` async return (it resolves an `AiResult`/`ApiResult`), **not** the synchronous `handle()` helper — which can only carry synchronous ops and would serialize a Promise (`DataCloneError`). It carries no `mutates` flag and emits no `vault:changed`. **Why:** the channel triggers model generation but changes no Vault state; a regression guard forbids any async `vault:*` channel from using the sync `handle()` helper so this class of bug cannot recur.
