...
**Phase 3 — AI + Project Truth Engine — ACTIVE.** Phase 2 baseline is locked (`v0.2.0`); the pre-Phase-3 release-readiness item BL-03 (recovery/backup) is complete. Owner approved Phase 3 planning on 2026-08-06; Phase 3 is delivered incrementally as `v0.3.0` → `v0.3.5`. **`v0.3.0` (AI Foundation) is complete and locked at tag `v0.3.0`.** **`v0.3.1` (Project Context & Repository Analysis) is now the ACTIVE slice (owner-approved 2026-08-06).** `v0.3.2`–`v0.3.5` remain planned, not active scope, and become active only when written into `.orbit/CURRENT_PHASE.md` with owner approval (Rule 3, root `AGENTS.md`). See `.orbit/CURRENT_PHASE.md` for the concrete `v0.3.1` objective, scope, exclusions, acceptance criteria, verification, and risks.
...
### v0.3.1 — Project Context & Repository Analysis 🔵 ACTIVE (owner-approved 2026-08-06)
...
- **Concrete active scope, exclusions, acceptance criteria, verification, and risks:** see `.orbit/CURRENT_PHASE.md` (the operational center). This slice is **analysis + context packaging only** — it must not drift into `v0.3.2` bootstrap/draft generation or `v0.3.3` proposal review, invoke a live model, or write canonical state.
